...
class AsciiFilter {
    ...
    constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: any = {}) {
        ...
        this.canvas.style.display = 'none'; // Hide intermediate canvas
    ...
    reset() {
        ...
        // Simplified CSS for stability
        this.pre.style.color = '#ff6b6b';
    ...
    hue() {
        // Hue rotation disabled for stability initially
    }
...
class CanvAscii {
    ...
    render() {
        ...
        if (this.textCanvas && this.texture) {
            this.textCanvas.render();
...
export default function ASCIIText({
    ...
    useEffect(() => {
        ...
        const initInstance = (container: HTMLElement, w: number, h: number) => {
        ...
        const setup = () => {
                    ...
                    ([entry]) => {
                        ...
                        if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
                            ...
                            if (!cancelled && containerRef.current && !asciiRef.current) {
                                asciiRef.current = initInstance(containerRef.current, w, h);
            ...
            if (!asciiRef.current) {
                asciiRef.current = initInstance(containerRef.current, width, height);
                asciiRef.current.load();
            }

            // Ensure explicit size set initially
        ...
        return () => {
            ...
            if (asciiRef.current) {
                try {
                    ...
                    console.error('Error disposing AsciiText', e);
                }
